...
impl<Tc:Hash+Eq+Clone,Tv> TabelaCV<Tc,Tv> {
    ...
    pub fn inserir(self:&mut Self, chave: Tc, valor:Tv) {
        ...
        match self.procurar_chave_hash(chave.clone(), hash_chave) {
            Some((end, _valor)) =>{
    ...
    pub fn remover(self: &mut Self, chave:Tc) {
        ...
        let hash_chave = self.calcular_hash(chave.clone());
        if let Some((end,_valor)) = self.procurar_chave_hash(chave.clone(), hash_chave) {
...
impl<Tc:Hash+Clone+Eq> ConjuntoIteravel<Tc> {
    ...
    pub fn inserir(self: &mut Self, elemento: Tc) {
        match self.tabela.ler(elemento.clone()) {
            None => {
    ...
    pub fn remover(self: &mut Self, elemento: Tc) {
        if let Some(endereco) = self.tabela.ler(elemento.clone()) {
...
//     //Essa funcao nao funciona porque a lista encadeada tem erro de heap corruption quando usamos uma string como entrada
...
//             println!("{}",palavra);
//             conj.acrescentar(palavra.clone());
...
//     let mut conj = Conjunto::<>::novo(2);
//     for s in ["a","Aarao","aba","abacate","abacateiro","abacateiros","abacates", "casa","lagartixa"] {
//         conj.acrescentar(s.to_string());
//     }
...
//     conj.acrescentar("Lagrange".to_string());
//     assert!(conj.contem("Lagrange".to_string()));
// }
#[test]
pub fn teste_conjunto() {
